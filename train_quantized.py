...
import tflite_quantize as tflite
...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    if rank in [-1, 0]:
        ...
        if opt.data.endswith('coco.yaml') and nc == 80:  # if COCO
            if opt.quantization:
                model_test = torch.load(best if best.exists() else last, map_location=device)['model'].float().eval()
                tflite.freeze_model(model_test)

            else:
                model_test = attempt_load(best if best.exists() else last, device)
