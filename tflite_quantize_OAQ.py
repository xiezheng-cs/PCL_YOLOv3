...
def uniform_symmetric_quantizer_per_tensor(x, alpha, bits=8, scale_bits=8, act_max=None):
    ...
    scale = scale * alpha  # overflow aware quantization
...
class Conv2d_quantization(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        self.register_buffer('activation_alpha', torch.ones(1))
        self.register_buffer('weight_alpha', torch.ones(1))
        self.register_buffer('scale_int_input', torch.zeros(1))
        self.register_buffer('scale_int_weight', torch.zeros(out_channels))
    ...
    def forward(self, input):
        if self.first_layer:
            ...
            quantized_input, self.scale_int_input = uniform_symmetric_quantizer_per_tensor(input, self.activation_alpha,
                                                    bits=self.quantization_bits, scale_bits=self.scale_bits, act_max=self.act_max)

        quantized_weight, self.scale_int_weight = uniform_symmetric_quantizer_per_channel(self.weight, self.weight_alpha,
                                                  bits=self.quantization_bits, scale_bits=self.scale_bits)

        if self.bias is not None:
            # quantization self.bias
            scale_int_bias = self.scale_int_input * self.scale_int_weight
            quantized_bias = uniform_symmetric_quantizer_per_channel(self.bias, bits=self.bias_bits, scale_bits=self.scale_bits,
                                                                     scale_int_bias=scale_int_bias)
        else:
            # not quantization self.bias
            quantized_bias = None

        return F.conv2d(quantized_input, quantized_weight, quantized_bias, self.stride, self.padding, self.dilation, self.groups)
...
if __name__ == "__main__":
    ...
    # print('quantzied_x.shape={}, \nquantzied_x={}'.format(quantzied_x.shape, quantzied_x))import torch
import torch.nn as nn
from torch.autograd import Function
from torch.nn import functional as F
import torch.distributed as dist
